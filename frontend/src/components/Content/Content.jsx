...
const Content = ({ sizeRange, strokeRange, searchQuery }) => {
  ...
  const totalPage = Math.ceil(filteredIcons.length / itemPerPage);

  const currentItems = filteredIcons.slice(
      ...
      <div className="col-span-5 lg:col-span-4">
        <div className="mb-4 flex items-center min-h-[28px]">
          <span className="text-sm font-medium text-gray-900 dark:text-white mr-3 shrink-0">
            ...
            {totalPage === currentPage
              ? iconList.length
              : currentPage * itemPerPage}{" "}
            icons of {iconList.length} in total.
          </span>
          <div class="flex items-center flex-wrap space-x-3">
            <span class="inline-flex items-center px-2 py-1 text-sm font-medium text-gray-900 bg-gray-100 rounded-md dark:bg-gray-800 dark:text-white">
              Solid
              <button
                type="button"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-3">
          {currentItems.map((val, ind) => (
            ...
            >
              <div className="absolute top-0 left-0 h-full w-full bg-[#1F2A37] rounded-lg flex items-center justify-center opacity-0 group-hover:opacity-80 transition-opacity">
                <div className="flex items-center">
                  ...
                  <span class="text-sm font-medium text-blue-600">copy</span>
                </div>
              </div>
